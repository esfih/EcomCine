...
`id`: `tools.server_side_converter.deploy.remote`
- goal: Build the standalone converter package, upload it to PlanetHoster over the saved SSH key, and provision or update the CloudLinux Node.js app for `converter.ecomcine.com`
- command: `./scripts/run-catalog-command.sh tools.server_side_converter.deploy.remote [deploy_version]`
- args: optional deploy version label; defaults to timestamp
- success: exit `0`; `https://converter.ecomcine.com/api/config` responds successfully after deployment
- failure: non-zero; inspect Passenger log under the remote app root and verify the CloudLinux Node.js app configuration
- failure_class: `auth`
- remediation_type: `source-fix`
...
`id`: `tools.server_side_converter.start`
- goal: Start the standalone native ffmpeg converter app outside WordPress for local or subdomain-style validation
- command: `./scripts/start-server-side-converter.sh [port]`
- args:
  - `[port]` — optional; default `9191`
- success: exit `0`; the app listens on `http://127.0.0.1:<port>` and serves `tools/server-side-converter/`
- failure: non-zero; verify `node` availability and converter folder integrity
- failure_class: `tooling`
- remediation_type: `source-fix`

`id`: `tools.server_side_converter.package`
- goal: Build a shareable standalone converter zip that bundles the current host platform's ffmpeg and ffprobe binaries inside the package
- command: `./scripts/build-server-side-converter-release.sh [version]`
- args:
  - `[version]` — optional; defaults to a timestamp-based build version
- success: exit `0`; outputs a zip and manifest under `dist/server-side-converter/`
- failure: non-zero; verify `ffmpeg`, `ffprobe`, and `zip` or `python3` are available on the host
- failure_class: `tooling`
- remediation_type: `source-fix`
