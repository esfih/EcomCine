...
if ( ! isset( $profile_info['address'] ) || ! is_array( $profile_info['address'] ) ) {
    $profile_info['address'] = [];
}
$profile_info['address'] = array_merge(
    [
        'street_1' => '',
        'street_2' => '',
        'city'     => '',
...
<form method="post" id="store-form" action="" class="dokan-form-horizontal tm-dokan-store-form">
    ...
    >
        ...
        <div class="button-area<?php echo esc_url( $banner_url ) ? ' dokan-hide' : ''; ?>">
            <?php echo TM_Icons::svg( 'cloud-upload-alt' ); ?>
    ...
    <div id="dokan-banner-video-wrapper" class="dokan-banner-video">
        <label class="dokan-control-label">
            <?php esc_html_e( 'Store Banner Video (Optional)', 'dokan-lite' ); ?>
            <span class="dokan-tooltips-help tips" title="<?php esc_attr_e( 'Upload a 15-second video to use as animated banner background. MP4 format recommended. Max 10MB.', 'dokan-lite' ); ?>">
        ...
        <div class="button-area<?php echo $banner_video_url ? ' dokan-hide' : ''; ?>">
            <?php echo TM_Icons::svg( 'video' ); ?>
            <a href="#" class="dokan-banner-video-drag dokan-btn dokan-theme dokan-btn-theme"><?php esc_html_e( 'Upload Video', 'dokan-lite' ); ?></a>
            <p class="help-block">
                <?php esc_html_e( 'Upload a short video (max 15 seconds, MP4 recommended, max 10MB). Video will loop as background. Banner image will be used as fallback/poster.', 'dokan-lite' ); ?>
            </p>
        </div>
    </div> <!-- .dokan-banner-video -->

    <?php if ( $banner_video_url ) : ?>
    <div class="dokan-form-group">
        <label class="dokan-w3 dokan-control-label" for="banner_video_position">
            <?php esc_html_e( 'Video Focus Position', 'dokan-lite' ); ?>
            <span class="dokan-tooltips-help tips" title="<?php esc_attr_e( 'Adjust where the video focuses (useful if subject/face is cut off)', 'dokan-lite' ); ?>">
                <?php echo TM_Icons::svg( 'question-circle' ); ?>
            </span>
        </label>
        <div class="dokan-w5">
            ...
            <select name="banner_video_position" id="banner_video_position" class="dokan-form-control">
                ...
                <option value="top" <?php selected( $video_position, 'top' ); ?>><?php esc_html_e( 'Top Center', 'dokan-lite' ); ?></option>
                <option value="bottom" <?php selected( $video_position, 'bottom' ); ?>><?php esc_html_e( 'Bottom Center', 'dokan-lite' ); ?></option>
                <option value="left" <?php selected( $video_position, 'left' ); ?>><?php esc_html_e( 'Center Left', 'dokan-lite' ); ?></option>
                <option value="right" <?php selected( $video_position, 'right' ); ?>><?php esc_html_e( 'Center Right', 'dokan-lite' ); ?></option>
                <option value="top left" <?php selected( $video_position, 'top left' ); ?>><?php esc_html_e( 'Top Left', 'dokan-lite' ); ?></option>
                <option value="top right" <?php selected( $video_position, 'top right' ); ?>><?php esc_html_e( 'Top Right', 'dokan-lite' ); ?></option>
                <option value="bottom left" <?php selected( $video_position, 'bottom left' ); ?>><?php esc_html_e( 'Bottom Left', 'dokan-lite' ); ?></option>
                <option value="bottom right" <?php selected( $video_position, 'bottom right' ); ?>><?php esc_html_e( 'Bottom Right', 'dokan-lite' ); ?></option>
            </select>
        </div>
    </div>
    <?php endif; ?>
    ...
    <div class="dokan-form-group">
        ...
        <div id="dokan-profile-picture-wrapper" class="dokan-w5 dokan-gravatar">
            ...
            <div class="gravatar-button-area<?php echo esc_url( $gravatar_url ) ? ' dokan-hide' : ''; ?>">
                <a href="#" class="dokan-pro-gravatar-drag dokan-btn dokan-btn-default"><?php echo TM_Icons::svg( 'cloud-upload-alt' ); ?> <?php esc_html_e( 'Upload Photo', 'dokan-lite' ); ?></a>
            </div>
        </div>
    </div>
    ...
    <div class="dokan-form-group talent_height tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_height" id="talent_height">
                <?php
                $current_height = get_user_meta( $vendor_id, 'talent_height', true );
                foreach ( $attributes_options['height'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
                        selected( $current_height, $value, false ),
                        esc_html( $label )
                    );
                }
                ?>
    ...
    <div class="dokan-form-group talent_weight tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_weight" id="talent_weight">
                <?php
                $current_weight = get_user_meta( $vendor_id, 'talent_weight', true );
                foreach ( $attributes_options['weight'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
    ...
    <div class="dokan-form-group talent_waist tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_waist" id="talent_waist">
                <?php
                $current_waist = get_user_meta( $vendor_id, 'talent_waist', true );
                foreach ( $attributes_options['waist'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
                        selected( $current_waist, $value, false ),
                        esc_html( $label )
                    );
                }
                ?>
    ...
    <div class="dokan-form-group talent_hip tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_hip" id="talent_hip">
                <?php
                $current_hip = get_user_meta( $vendor_id, 'talent_hip', true );
                foreach ( $attributes_options['hip'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
                        selected( $current_hip, $value, false ),
                        esc_html( $label )
                    );
                }
                ?>
    ...
    <div class="dokan-form-group talent_chest tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_chest" id="talent_chest">
                <?php
                $current_chest = get_user_meta( $vendor_id, 'talent_chest', true );
                foreach ( $attributes_options['chest'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
                        selected( $current_chest, $value, false ),
                        esc_html( $label )
                    );
                }
                ?>
    ...
    <div class="dokan-form-group talent_shoe_size tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_shoe_size" id="talent_shoe_size">
                <?php
                $current_shoe = get_user_meta( $vendor_id, 'talent_shoe_size', true );
                foreach ( $attributes_options['shoe_size'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
                        selected( $current_shoe, $value, false ),
                        esc_html( $label )
                    );
                }
                ?>
    ...
    <div class="dokan-form-group talent_eye_color tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_eye_color" id="talent_eye_color">
                <?php
                $current_eye = get_user_meta( $vendor_id, 'talent_eye_color', true );
                foreach ( $attributes_options['eye_color'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
                        selected( $current_eye, $value, false ),
                        esc_html( $label )
                    );
                }
                ?>
    ...
    <div class="dokan-form-group talent_hair_color tm-hidden-by-default" data-category="model,artist">
        ...
        <div class="dokan-w5">
            <select class="dokan-form-control" name="talent_hair_color" id="talent_hair_color">
                <?php
                $current_hair_color = get_user_meta( $vendor_id, 'talent_hair_color', true );
                foreach ( $attributes_options['hair_color'] as $value => $label ) {
                    printf(
                        '<option value="%s" %s>%s</option>',
                        esc_attr( $value ),
                        selected( $current_hair_color, $value, false ),
                        esc_html( $label )
                    );
                }
                ?>
