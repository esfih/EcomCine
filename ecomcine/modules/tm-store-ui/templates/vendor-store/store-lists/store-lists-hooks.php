 ...
 */
function tm_store_list_sort_args( $tm_order = 'oldest' ) {
	switch ( $tm_order ) {
		case 'oldest':
...
function filter_dokan_seller_listing_args( $args ) {
	...
	// ── Sort order ────────────────────────────────────────────────────────────
	$_tm_order = isset( $_GET['tm_order'] ) ? sanitize_key( $_GET['tm_order'] ) : 'oldest';
	...
	(function($) {
		...
		(function() {
			...
			if ( window.tmShowcaseData && window.tmShowcaseData.ids && window.tmShowcaseData.ids.length ) {
				var activeSortKey = new URLSearchParams( window.location.search ).get( 'tm_order' ) || 'oldest';
				var sortOptions   = [
