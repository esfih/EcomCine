...
if ( ! function_exists( 'tm_store_ui_render_cinematic_header' ) ) {
	function tm_store_ui_render_cinematic_header( $force = false ) {
	...
	if ( ! $force && ! $on_store_page && ! $on_store_listing && ! tm_is_showcase_page() && ! $on_platform ) { return; }
	...
	$header_socials      = class_exists( 'EcomCine_Admin_Settings', false ) ? EcomCine_Admin_Settings::get_header_socials() : array();
	$header_social_label = isset( $header_socials['label'] ) ? trim( (string) $header_socials['label'] ) : 'Follow us';
	$header_social_items = isset( $header_socials['items'] ) && is_array( $header_socials['items'] ) ? $header_socials['items'] : array();
	$header_social_links = array();

	foreach ( $header_social_items as $social_item ) {
		if ( ! is_array( $social_item ) || empty( $social_item['enabled'] ) ) {
			continue;
		}

		$icon_key = isset( $social_item['icon'] ) ? sanitize_key( (string) $social_item['icon'] ) : '';
		$label    = isset( $social_item['label'] ) ? trim( (string) $social_item['label'] ) : '';
		...
		if ( class_exists( 'TM_Icons' ) ) {
			$icon_markup = TM_Icons::svg( $icon_key, 'social-icon-svg', $label );
		} else {
			switch ( $icon_key ) {
				case 'youtube':
					$icon_markup = '<i class="fab fa-youtube" aria-hidden="true"></i>';
					break;
				case 'facebook':
					$icon_markup = '<i class="fab fa-facebook" aria-hidden="true"></i>';
					break;
				case 'instagram':
					$icon_markup = '<i class="fab fa-instagram" aria-hidden="true"></i>';
					break;
				case 'linkedin':
					$icon_markup = '<i class="fab fa-linkedin" aria-hidden="true"></i>';
					break;
				case 'x-twitter':
				default:
					$icon_markup = '<i class="fab fa-x-twitter" aria-hidden="true"></i>';
					break;
			}
		}

		$header_social_links[] = '<a class="social-icon" href="' . $url . '" target="_blank" rel="noopener noreferrer" aria-label="' . esc_attr( $label ) . '">' . $icon_markup . '</a>';
	}

	$header_socials_html = '';
	if ( ! empty( $header_social_links ) ) {
		$header_socials_html = '<div class="tm-header-platforms" aria-label="' . esc_attr__( 'Follow us on social media', 'tm-store-ui' ) . '">'
			. '<span class="tm-header-platforms__label">' . esc_html( $header_social_label ) . '</span>'
			. '<span class="tm-header-platforms__icons">' . implode( '', $header_social_links ) . '</span>'
			. '</div>';
	}
	?>
...
add_action( 'wp_body_open', function() {
	...
	tm_store_ui_render_cinematic_header();
}, 5 );
