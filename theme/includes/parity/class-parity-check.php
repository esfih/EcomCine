 ...
 *
 * Usage (via WP-CLI):
 *   wp eval 'require ABSPATH . "wp-content/themes/astra-child/includes/parity/class-parity-check.php"; THO_Parity_Check::run();' --allow-root
 *
...
class THO_Parity_Check {
	...
	private function run_suite( string $mode ): void {
		...
		$this->check( $mode, 'template_router_store_returns_string', function () {
			$result = THO_Adapter_Registry::get_template_router()->get_store_page_template( [] );
		...
		$this->check( $mode, 'template_router_listing_returns_string', function () {
			$result = THO_Adapter_Registry::get_template_router()->get_listing_page_template( [] );
		...
		$this->check( $mode, 'asset_policy_returns_array_with_required_keys', function () {
			$result = THO_Adapter_Registry::get_asset_policy_provider()->get_asset_policy( [] );
	...
	private function check( string $adapter, string $name, callable $fn ): void {
		try {
			$fn();
			$this->pass++;
			$this->results[] = [ 'adapter' => $adapter, 'check' => $name, 'status' => 'PASS' ];
		} catch ( \Throwable $e ) {
			$this->fail++;
			$this->results[] = [ 'adapter' => $adapter, 'check' => $name, 'status' => 'FAIL', 'error' => $e->getMessage() ];
		}
	}
}
