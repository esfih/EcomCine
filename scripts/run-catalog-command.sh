...
case "$COMMAND_ID" in
  ...
  tools.server_side_converter.start)
    bash ./scripts/start-server-side-converter.sh "$@"
    ;;

  tools.server_side_converter.package)
    bash ./scripts/build-server-side-converter-release.sh "$@"
    ;;

  tools.server_side_converter.deploy.remote)
    bash ./scripts/deploy-server-side-converter-remote.sh "$@"
    ;;
