...
// Models that don't support think/reasoning_effort injection — always passthrough
const PASSTHROUGH_MODEL_PATTERNS = [
  /gemma/i,
];

function modelRequiresPassthrough(modelName) {
  if (!modelName || typeof modelName !== "string") return false;
  return PASSTHROUGH_MODEL_PATTERNS.some(re => re.test(modelName));
}
...
async function start() {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      const isPassthroughModel = parsed && modelRequiresPassthrough(parsed.model);
      if (isPassthroughModel) {
        log("PASSTHROUGH", pathname, parsed.model, "(model excluded from injection)");
      }

      if (parsed && shouldInject(pathname) && !isPassthroughModel) {
      ...
      } else if (parsed && isPassthroughModel && shouldInject(pathname)) {
        // For passthrough models: apply tool compression + keep_alive, but skip nothink injection
        const compStats = compressRequest(parsed);
        if (compStats.toolsRemoved > 0 || compStats.bytesEstimate > 0) {
          log(
            "COMPRESS",
        ...
        if ((pathname === "/api/chat" || pathname === "/api/generate") && parsed.keep_alive == null) {
          parsed.keep_alive = -1;
        }
        if (predictCap > 0 && parsed.max_tokens == null && pathname !== "/api/chat" && pathname !== "/api/generate") {
          parsed.max_tokens = predictCap;
        }
        log("FWD", pathname, parsed.model, parsed.think, parsed.stream, "passthrough(compressed)");
        outgoingBody = Buffer.from(JSON.stringify(parsed), "utf8");
      }
